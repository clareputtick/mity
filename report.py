...
def make_table(variants, samples, vep_headers, impact_dict, min_vaf):
    ...
    for lineparts in variants:
        ...
        if vep_headers != "":
            ...
            # get the index of where the consequence sits in the list of VEP annotations
            ...
            # remove the sublists that have 'stream'in the consequence from the VEP annotations
            ...
            if VEP == []:
                ...
                VEP = [[element or '.' for element in sublist] for sublist in VEP]
            ...
            # concatenate the vep annotations together if there is more than one annotation per variant
            vep_annotations = []
        ...
        for samp in range(0, n_sample):
            ...
            if float(FORMAT_VAF) > float(min_vaf):
                ...
                if float(FORMAT_VAF) >= 0.01:
                    ...
                    # table = [["SAMPLE", "CHR", "POS", "REF", "ALT", "QUAL", "FILTER", "INFO_DP", "INFO_MQM", "INFO_MQMR", 
                    #       "INFO_QA", "INFO_QR", "INFO_SAF", "INFO_SAR", "INFO_SRF", "INFO_SRR", "INFO_SBR", "INFO_SBA", 
                    #       "INFO_VAF", "INFO_POS_FIL", "INFO_SBR_FIL", "INFO_SBA_FIL", "INFO_MQMR_FIL", "INFO_AQR_FIL",
                    #       "FORMAT_GT", "FORMAT_DP", "FORMAT_RO", "FORMAT_QR", "FORMAT_AQR", "FORMAT_AO", 
                ...
                new_line = [samples[samp], chromosome, pos, ref, alt, hgvs, QUAL, FILTER, INFO_DP, INFO_MQM, INFO_MQMR, 
                    INFO_QA, INFO_QR, INFO_SAF, INFO_SAR, INFO_SRF, INFO_SRR, INFO_SBR, INFO_SBA, 
                    FORMAT_VAF, POS_FIL, SBR_FIL, SBA_FIL, MQMR_FIL, AQR_FIL, 
                    FORMAT_GT, FORMAT_DP, FORMAT_RO, FORMAT_QR, FORMAT_AQR, FORMAT_AO, 
                    FORMAT_QA, FORMAT_AQA, tier, no_comma_info, no_comma_format]
...
def split_header_variants(vcf):
        ...
        if not (col_names[0] == '#' and col_names[1] != '#'):
            sys.exit("It looks like the last line of the header is not the column names.")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--minVAF', action='store', type = float, default = 0, help = 'A variant must have at least this VAF to be included in the report. Default: 0.')
    parser.add_argument('vcf', action = 'append', nargs='+', help = 'VCF files to make the report on.')
    ...
    #   minVAF = float(minVAF)
    # except ValueError:
    #   sys.exit(" Input: minVAF=" + str(minVAF) + "\n Bad input, <min_VAF> must convert to a float \n Usage: python3 make_mity_variant_table.py <min_VAF> <VCFs>") 
    ...
    if len(bad_vcf_index) > 0:
        ...
        # err_msg = err_msg + "Usage: python3 make_mity_variant_table.py <min_VAF> <VCFs>"
        sys.exit(err_msg)
    ...
    for filepath in vcf:
        ...
        # for each file get the first line that doesnt start with # to get the sample names
    ...
    # if the number of unique IDs does not match the number of unique descriptions, then there is one ID 
    # that has more than one descrption, either because of one vcf with duplicated headers, or because
    ...
    # print(samples)
    if len(samples) != len(col_names) != len(variants) != len(is_vepped) != len(vep_header):
        sys.exit("length of samples, col_names, variants, is_vepped, vep_header is different, something odd has happened.")
    ...
    if len(col_names) > 1:
        ...
        if set(all_equal) != set([True]):
            sys.exit("VCF inputs do not have the same column name for the variants")
    ...
    # if all the vep_header are the same we only need to keep one of them
    ...
    for vcf_num in range(0,len(variants)):
        v = make_table(variants[vcf_num], samples[vcf_num], vep_header, impacts, minVAF)
    ...
    # note that this is hard coded and the function make_table outputs the columns in this order
    # the column order is changed later in the code
    vcf_headers = ["SAMPLE", "CHR", "POS", "REF", "ALT", "HGVS", "QUAL", "FILTER", "total_locus_depth", "MQM_INFO", "MQMR_INFO", 
            "QA_INFO", "QR_INFO", "SAF_INFO", "SAR_INFO", "SRF_INFO", "SRR_INFO", "SBR_INFO", "SBA_INFO", 
            "variant_heteroplasmy", "POS_FIL", "SBR_FIL", "SBA_FIL", "MQMR_FIL", "AQR_FIL",
            "GT_FORMAT", "total_sample_depth", "ref_depth", "QR_FORMAT", "AQR_FORMAT", "alt_depth", 
    ...
    mitomap_panel_annotated_variants = pandas.merge(left = variant_df, right = mitomap_panel_annotations, how='left', on=['CHR', 'POS', 'REF', 'ALT'])
    # mitomap_panel_annotated_variants.to_csv("python_annotated_variants.csv", index = False)
    ...
    gtf_mitomap_panel_annotated_variants = pandas.merge(left = mitomap_panel_annotated_variants, right = gtf_annotations, how='left', on=['CHR', 'POS'])
    ...
    mito_locus_gtf_mitomap_panel_annotated_variants = pandas.merge(left = gtf_mitomap_panel_annotated_variants, right = mito_locus_annotations, how='left', on=['CHR', 'POS'])
    ...
    mitotip_trna_mito_locus_gtf_mitomap_panel_annotated_variants = pandas.merge(left = trna_mito_locus_gtf_mitomap_panel_annotated_variants, right = mitotip_annotations, how='left', on=['CHR', 'POS', 'REF', 'ALT'])
    # for line in trna_mito_locus_gtf_mitomap_panel_annotated_variants:
    #   print(line)
    # sys.exit()
    ...
    annotated_variants = haplotype_mgrb_mitotip_trna_mito_locus_gtf_mitomap_panel_annotated_variants.fillna('.')
    ...
    cohort_count_table = pandas.DataFrame({'count' : annotated_variants.groupby( [ "POS", "ALT"] ).size()}).reset_index()
    cohort_count_table.columns = ['POS', 'ALT', 'COHORT_COUNT']
    

    annotated_variants1 = pandas.merge(left = annotated_variants, right = cohort_count_table, how='left', on=['POS', 'ALT'])
    

    # add in the GB percent
    # need to account for the fact that some entries will be "." and cant be converted to float
    ...
    # otherwise dont
    if is_vepped:
        cols = ['SAMPLE','HGVS', 'gene/locus', 'gene/locus description', 'variant_heteroplasmy', 
                'ref_depth', 'alt_depth', 'total_sample_depth', 'total_locus_depth', 'COHORT_COUNT', 'tier',
                'baylor_panel', 'common_22_panel', 'common_58_panel',
                'MGRB_frequency', 'phylotree_haplotype',  'MitoTip_score', 'MitoTip_score_percentile', 'anticodon', 'allele_frequency_mitomap',
                'highest_vep_impact', 'Consequence_VEP', 'disease_mitomap',
                'MGRB_FILTER', 'MGRB_AN', 'MGRB_AC','phylotree_mut',
                'Codons_VEP', 'Amino_acids_VEP', 'Gene_VEP', 'SYMBOL_VEP', 'Feature_VEP', 'EXON_VEP', 'PolyPhen_VEP', 'SIFT_VEP',
                'Protein_position_VEP', 'BIOTYPE_VEP', 'CANONICAL_VEP', 'Feature_type_VEP', 'cDNA_position_VEP', 'CDS_position_VEP',
                'Existing_variation_VEP', 'DISTANCE_VEP', 'STRAND_VEP', 'CLIN_SIG_VEP', 'LoF_flags_VEP', 'LoF_filter_VEP', 'LoF_VEP',
                'RadialSVM_score_VEP', 'RadialSVM_pred_VEP', 'LR_score_VEP', 'LR_pred_VEP', 'CADD_raw_VEP', 'CADD_phred_VEP', 'Reliability_index_VEP',
                'HGVSc_VEP', 'HGVSp_VEP',
                'locus_mitomap', 'variant_references_mitomap', 'variant_amino_acid_change_mitomap', 'codon_position_mitomap', 'codon_number_mitomap',
                'disease_references_mitomap', 'RNA_mitomap', 'homoplasmy_mitomap', 'heteroplasmy_mitomap', 'status_mitomap', 'disease_amino_acid_change_mitomap', 'GB_frequency_mitomap',
                'CHR', 'POS', 'REF', 'ALT', 'QUAL', 'FILTER',
                'MQM_INFO', 'MQMR_INFO', 'QA_INFO', 'QR_INFO', 'SAF_INFO', 'SAR_INFO', 'SRF_INFO', 'SRR_INFO', 'SBR_INFO', 'SBA_INFO',
                'POS_FIL', 'SBR_FIL', 'SBA_FIL', 'MQMR_FIL', 'AQR_FIL',
                'GT_FORMAT', 'QR_FORMAT', 'AQR_FORMAT', 'QA_FORMAT', 'AQA_FORMAT',
                'INFO', 'FORMAT']
    else:   
        cols = ['SAMPLE','HGVS', 'gene/locus', 'gene/locus description', 'variant_heteroplasmy', 
                'ref_depth', 'alt_depth', 'total_sample_depth', 'total_locus_depth', 'COHORT_COUNT', 'tier',
                'baylor_panel', 'common_22_panel', 'common_58_panel',
                'MGRB_frequency', 'phylotree_haplotype',  'MitoTip_score', 'MitoTip_score_percentile', 'anticodon', 'allele_frequency_mitomap',
                'disease_mitomap', 'MGRB_FILTER', 'MGRB_AN', 'MGRB_AC','phylotree_mut',
                'locus_mitomap', 'variant_references_mitomap', 'variant_amino_acid_change_mitomap', 'codon_position_mitomap', 'codon_number_mitomap',
                'disease_references_mitomap', 'RNA_mitomap', 'homoplasmy_mitomap', 'heteroplasmy_mitomap', 'status_mitomap', 'disease_amino_acid_change_mitomap', 'GB_frequency_mitomap',
                'CHR', 'POS', 'REF', 'ALT', 'QUAL', 'FILTER',
                'MQM_INFO', 'MQMR_INFO', 'QA_INFO', 'QR_INFO', 'SAF_INFO', 'SAR_INFO', 'SRF_INFO', 'SRR_INFO', 'SBR_INFO', 'SBA_INFO',
    ...
    if len(annotated_variants1.columns.tolist()) != len(cols):
        ...
        print("")
        print("annotated_variants missing"+str(set(annotated_variants.columns.tolist())-set(cols)))
        print("")
        print("cols"+str(set(cols)-set(annotated_variants.columns.tolist())))
    ...
    documentation.append("HGVS: Variant annotation using HGVS syntax, which assumes variant is SNP, insertion or deletion")
    documentation.append("gene/locus: The gene that the variant falls in, or if it is not in a gene then the mitomap entry, if it exists.")
    documentation.append("variant_heteroplasmy: alt_depth/(ref_depth+alt_depth)")
    documentation.append("total_sample_depth: alt_depth + ref_depth for the particular sample")
    documentation.append("total_locus_depth: sum of all the total_sample_depth in the vcf file")
    documentation.append("COHORT_COUNT: The number of times that variant (defined by position and alternate) is seen in all of the input VCFs")
    documentation.append("tier: 1:=VAF>=1%. 2:= VAF<1% and alt_depth>=10, 3:=VAF<1% and alt_depth<10")
    documentation.append("baylor_panel: https://www.bcm.edu/research/medical-genetics-labs/test_detail.cfm?testcode=2010")
    documentation.append("common_22_panel: https://www.vcgs.org.au/order/tests/634")
    documentation.append("common_58_panel: https://www.genedx.com/test-catalog/available-tests/58-confirmed-disease-causing-mtdna-point-mutations-and-deletion-testing/")
    documentation.append("MGRB_frequency: The variant frequency in MGRB")
    documentation.append("phylotree_haplotype: The haplotypes this mutation contributes too. See http://www.phylotree.org/index.htm")
    documentation.append("MitoTip_score: Only for variants in a tRNA. The score is a preditction of the pathogenicity of variants in tRNAs, see https://www.mitomap.org/foswiki/bin/view/MITOMAP/MitoTipInfo. The highest score is 21.8.")
    documentation.append("MitoTip_score_percentile: Where this variants MitoTip score sits in relation to all of the possible variants in tRNA. 100% is the most pathogenic, 0% is a score of zero.")
    documentation.append("anticodon: TRUE if the variant falls in an anticodon of a tRNA.")
    documentation.append("allele_frequency_mitomap: Mitomap derives this from 32059 GenBank sequences with size greater than 15.4kbp (range 0-1)")
    documentation.append("highest_vep_impact: Higest VEP impact from all the VEP annotations for the variant. See http://www.ensembl.org/info/docs/tools/vep/index.html")
    documentation.append("Columns suffix with _mitomap are annotations drawn from the mitomap website http://www.mitomap.org/MITOMAP.")
    documentation.append("In particular the 4 tables Control Region Variants (16024-576), Coding & RNA Variants (577-16023, MTTF-MTTP), rRNA/tRNA Mutations and Coding & Control Region Mutations")
    documentation.append("Columns suffix with _VEP come from Ensembl’s Variant Effect Predictor (VEP) annotations in the VCF")
    documentation.append("Entries separated with a semicolon mean that there are multiple annotations, from either VEP or mitomap.")
    documentation.append("For example if there are two entries in the mitomap columns, x;y all the annotations before the ';'' are related, and all the annotations after are related.")
    documentation.append("Fields from VCF header:")

    if dupe_header_warning:
        documentation.append("Warning: VCF header IDs are not unique, so there might be a repeated definition below.")
    ...
    annotated_variants1['variant_heteroplasmy'] = annotated_variants1['variant_heteroplasmy'].astype('float64')
    annotated_variants1['ref_depth'] = annotated_variants1['ref_depth'].astype('int64')
    annotated_variants1['alt_depth'] = annotated_variants1['alt_depth'].astype('int64')
    annotated_variants1['total_sample_depth'] = annotated_variants1['total_sample_depth'].astype('int64')
    annotated_variants1['total_locus_depth'] = annotated_variants1['total_locus_depth'].astype('int64')
    annotated_variants1['COHORT_COUNT'] = annotated_variants1['COHORT_COUNT'].astype('int64')
    annotated_variants1['tier'] = annotated_variants1['tier'].astype('int64')
    annotated_variants1['MQM_INFO'] = annotated_variants1['MQM_INFO'].astype('float64')
    annotated_variants1['MQMR_INFO'] = annotated_variants1['MQMR_INFO'].astype('float64')
    annotated_variants1['QA_INFO'] = annotated_variants1['QA_INFO'].astype('float64')
    annotated_variants1['QR_INFO'] = annotated_variants1['QR_INFO'].astype('float64')
    annotated_variants1['SAF_INFO'] = annotated_variants1['SAF_INFO'].astype('float64')
    annotated_variants1['SAR_INFO'] = annotated_variants1['SAR_INFO'].astype('float64')
    annotated_variants1['SRF_INFO'] = annotated_variants1['SRF_INFO'].astype('float64')
    annotated_variants1['SRR_INFO'] = annotated_variants1['SRR_INFO'].astype('float64')
    annotated_variants1['SBR_INFO'] = annotated_variants1['SBR_INFO'].astype('float64')
    annotated_variants1['SBA_INFO'] = annotated_variants1['SBA_INFO'].astype('float64')

    xlsx_name = prefix + '.annotated_variants.xlsx'
    writer = pandas.ExcelWriter(xlsx_name, engine='xlsxwriter')
    documentation_df.to_excel(writer, sheet_name='Documentation',index=False, header=False)
