...
def do_call(bam_files, reference, prefix=None, sites=None, min_mq=30, min_bq=20, min_af=0.5, min_ac=4, ploidy=2):
    ...
    freebayes_call = (f'freebayes -f {reference} {bam_str} -r {region} ' +
       f'--min-mapping-quality {min_mq} --min-base-quality {min_bq} ' +
       f'--min-alternate-fraction {min_af} --min-alternate-count {min_ac} ' +
    ...
    # run mity normalise
    #norm_vcf = normalise.do_call('unnormalised.vcf.gz', 'MT', output_file_name)
    mity_normalise_call = "~/src/DNANexus_Applets/kccg-mity-call/resources/home/dnanexus/mity_normalise.py --vcf unnormalised.vcf.gz | sort -k1,1 -k2,2n | bgzip > " + output_file_name
